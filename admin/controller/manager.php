...
elseif ($action == "edit") :
    ...
    if (empty($name) || strlen($name) < 5) {
        ...
        $icon     = "error";
    } elseif ($conn->query("SELECT * FROM admins WHERE username!='$username' && email='$email' ")->rowCount()) {
        $error      = 1;
        ...
        $insert1 = $conn->prepare("UPDATE admins SET name=:name, email=:email, telephone=:telephone ,
        access=:access , client_type=:client_type WHERE admin_id=:id ");
        $insert1 = $insert1->execute(array(
            "id" => $admin_id, "name" => $name, "email" => $email, "telephone" => $telephone,
...
elseif ($action == "new") :
    ...
    if (empty($name) || strlen($name) < 5) {
        ...
        access=:access , username=:username , password=:password ,client_type=:client_type , admin_type=:admin_type");
        $insert2 = $insert2->execute(array(
